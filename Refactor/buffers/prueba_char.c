...
ver_info (CDKMATRIX * matriz)
{
  ...
  int size = 4;
  int rta =
    viewInfo (ScreenOf (matriz), title, info, size, buttons, buttonCount,
	      FALSE);
  if (rta == 1)
    {
...
void
sacar_info (CDKSCREEN * pantalla, CDKMATRIX * matriz)
{
  int i;
...
//      tmp[i]=(char*)malloc(sizeof("sdfdsfsdfdfdsdfdsf"));
  ...
  buffer[6] = "asdfsdf";
  buffer[7] = "asdfsdf";
  printf ("%s,void=%d,char**=%d", buffer[4], sizeof (void *),
	  sizeof (char **));
