...
int
call_tabla (void *nombre, int argc, char **argv, char **azColName)
{
  char tmp[255];
...
int
call_registro (void *nombre, int argc, char **argv, char **azColName)
{
  int i;
//printf("%s\n",argv[1]);
  int columnas[122];
  matriz = formulario (nombre);
  printf ("longitud argv= %d  matriz->rows=%d", lenCharList (argv),
	  matriz->rows);
  for (i = 0; i < argc; i++)
    {
      printf ("%s\n", argv[i]);
...
void
ver_registro (char *tabla, char *campo, char *valor)
{
  char *error;
  char consulta[255];
...
int
call_columnas (void *NotUsed, int argc, char **argv, char **azColName)
{
