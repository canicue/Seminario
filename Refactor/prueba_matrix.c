...
#define set_row(n, string) \
  ...
  if (form_prov == 0)
    {
      ...
      printf
	("Oops. Can't seem to create the matrix widget. Is the window too small ?\n");
      exit (EXIT_FAILURE);
    }
  ...
  if (form_prov->exitType == vESCAPE_HIT)
  ...
  else if (form_prov->exitType == vNORMAL)
    {
      ...
      sprintf (temp, "Current cell (%d,%d)", form_prov->crow,
	       form_prov->ccol);
      ...
      mesg[4] =
	getCDKMatrixCell (form_prov, form_prov->crow, form_prov->ccol);
      mesg[5] = "";
